	...
	for _, name := range names {
		...
		default:
			...
			return nil, ghErrors.NewStructuredResolutionError(
				"field_ambiguous",
				name,
				fmt.Sprintf("multiple fields share this name; pass numeric IDs via %q to disambiguate", idParameter),
				candidates,
			)
		}
	}
...
func resolveFieldNamesToIDs(ctx context.Context, gqlClient *githubv4.Client, owner, ownerType string, projectNumber int, names []string) ([]int64, error) {
	...
	resolved, err := resolveFieldsByName(all, owner, projectNumber, names, "fields")
	if err != nil {
		return nil, err
	}
	...
	for i, field := range resolved {
		...
		if parseErr != nil {
			return nil, fmt.Errorf("resolved field %q has non-numeric ID %q; pass it via 'fields' instead", names[i], field.ID)
		}
		out = append(out, id)
	}
	return out, nil
